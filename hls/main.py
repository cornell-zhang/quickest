...
def sweep_params(hyperparam_list, train_val_pairs, data_pack, model_name):
  ...
  # test
  test_RAE0 = model_testing(clf_models_RAE, test_X, test_Y, normVal['training_Y'], Y_normalized = False)

  # print result
  print model_name, test_RAE0, "(", best_RAE_param, ")"

  return clf_models_RAE, test_RAE0
...
def run_all_data(all_data, dev_data, names):
  ...
  model_params['ann'] = gen_ANN_param_list(max_size_1D = 20, max_size_2D = 10)
  model_params['multiann'] = model_params['ann']

  # generate XGBoost param list
  model_params['xgb'] = gen_XGB_param_list(max_depth = 1, max_trees = 200)
  ...
  for model in model_list:
    ...
    clf_models_RAE, all_test_RAE = sweep_params(model_params[model], all_train_val_pairs, all_data, model)
    all_results[model] = np.array(all_test_RAE)
    ...
    for device in [0, 1, 2, 3]:
      test_RAE0 = model_testing(clf_models_RAE, dev_data[device]['test_X'], 
                                dev_data[device]['test_Y'], all_data['normVal']['training_Y'], Y_normalized = False)
      print model, "on test", device, ":", test_RAE0
      all_results[model+str(device)] = np.array(test_RAE0)
...
def predict_timing(all_data, dev_data, names):
  ...
  for model in ['xgb', 'ann']:
    ...
    clf.fit(all_data['training_X'], new_training_Y_all.reshape(new_training_Y.shape[0],))
    # test the best model on individual device test sets
    for device in [0, 1, 2, 3]:
      ...
      print "Test ", device, "error: ", test_error
